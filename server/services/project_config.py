...
Configuration is stored in {project_dir}/.xaheen/config.json.
"""
...
def _get_config_path(project_dir: Path) -> Path:
    ...
    Checks the new .xaheen/ location first, falls back to .autocoder/
    ...
    """
    new_path = project_dir / ".xaheen" / "config.json"
    if new_path.exists():
        return new_path
...
def _save_config(project_dir: Path, config: dict) -> None:
    ...
    Creates the .xaheen directory if it doesn't exist.
    ...
    # Ensure the .xaheen directory exists
...
def clear_dev_command(project_dir: Path) -> None:
    ...
    if not config:
        try:
            ...
            # Also remove .xaheen directory if empty
            xaheen_dir = config_path.parent
            if xaheen_dir.exists() and not any(xaheen_dir.iterdir()):
                xaheen_dir.rmdir()
                logger.debug("Removed empty .xaheen directory for %s", project_dir.name)
        except OSError as e:
