 ...
 * ==========
 *
 * Main CLI module for the Xaheen npm global package.
...
/** Root of the xaheen npm package (one level up from lib/) */
const PKG_DIR = dirname(dirname(fileURLToPath(import.meta.url)));

/** User config home: ~/.xaheen/ (falls back to ~/.autoforge/ for migration) */
const CONFIG_HOME_NEW = join(homedir(), '.xaheen');
const CONFIG_HOME_LEGACY = join(homedir(), '.autoforge');
// Migrate: if old dir exists but new doesn't, use old (will be migrated on next venv setup)
const CONFIG_HOME = existsSync(CONFIG_HOME_NEW) ? CONFIG_HOME_NEW
  : existsSync(CONFIG_HOME_LEGACY) ? CONFIG_HOME_LEGACY
    : CONFIG_HOME_NEW;
 ...
 *
 * The XAHEEN_PYTHON (or legacy AUTOFORGE_PYTHON) env var overrides automatic detection.
 *
 * After finding a candidate we also verify that the venv module is
 * available (Debian/Ubuntu strip it out of the base package).
 */
function findPython() {
  // Allow explicit override via environment variable (new name, fallback to old)
  const override = process.env.XAHEEN_PYTHON || process.env.AUTOFORGE_PYTHON;
  if (override) {
    const result = tryPythonCandidate(override);
    if (!result) {
      die(`XAHEEN_PYTHON is set to "${override}" but it could not be executed.`);
    }
    if (result.tooOld) {
      die(
        `Python ${result.version.raw} found (via XAHEEN_PYTHON), but 3.11+ required.\n` +
...
function ensureVenv(python, forceRecreate) {
  ...
  if (needsCreate) {
    ...
    log(`      Creating virtual environment at ~/${CONFIG_HOME === CONFIG_HOME_NEW ? '.xaheen' : '.autoforge'}/venv/`);
...
/**
 * Ensure ~/.xaheen/.env exists. On first run, copy .env.example
...
function ensureEnvFile() {
  ...
  if (existsSync(ENV_EXAMPLE)) {
    ...
    writeFileSync(ENV_FILE, '# Xaheen configuration\n# See documentation for available options.\n', 'utf8');
  }
  return true;
}
...
function printHelp() {
  ...
  Usage:
    ...
    xaheen config --show            Show effective configuration
...
function startServer(opts) {
  ...
  if (existingPid && isProcessAlive(existingPid)) {
    log(`Xaheen is already running at http://${host}:${port}`);
    log('Opening browser...');
  ...
  if (configCreated) {
    log(`      Created config file: ~/.xaheen/.env`);
  ...
  if (host !== '127.0.0.1') {
    ...
    console.log('  The Xaheen UI will be accessible from other machines.');
  ...
  if (host !== '127.0.0.1') {
    serverEnv.XAHEEN_ALLOW_REMOTE = '1';
  }
