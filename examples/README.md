# Xaheen Security Configuration Examples
...
When you create a new project with Xaheen, it automatically creates:
...
cp examples/org_config.yaml ~/.xaheen/config.yaml

# Edit it to add org-wide commands
nano ~/.xaheen/config.yaml
```
...
**File:** `{project_dir}/.xaheen/allowed_commands.yaml`
...
└─────────────────────────────────────────────────────┘
                         ↓
┌─────────────────────────────────────────────────────┐
│ 2. ORG BLOCKLIST (~/.xaheen/config.yaml)         │
│    Commands you block organization-wide             │
│    ❌ Projects CANNOT override these                │
└─────────────────────────────────────────────────────┘
                         ↓
┌─────────────────────────────────────────────────────┐
│ 3. ORG ALLOWLIST (~/.xaheen/config.yaml)         │
...
└─────────────────────────────────────────────────────┘
                         ↓
┌─────────────────────────────────────────────────────┐
│ 5. PROJECT ALLOWLIST (.xaheen/allowed_commands)  │
...
**Project config** (`.xaheen/allowed_commands.yaml`):
```yaml
version: 1
commands:
  - name: swift*
...
```yaml
# In .xaheen/allowed_commands.yaml
commands:
  - name: X
...
**Solution:**
- If in org blocklist: Edit `~/.xaheen/config.yaml` to remove it
...
2. Verify file location:
   - Project: `{project}/.xaheen/allowed_commands.yaml`
   - Org: `~/.xaheen/config.yaml` (must be manually created)
...
Xaheen has comprehensive tests for the security system:
...
# Edit the config
nano .xaheen/allowed_commands.yaml
```
...
To allow this command:
  1. Add to .xaheen/allowed_commands.yaml for this project, OR
