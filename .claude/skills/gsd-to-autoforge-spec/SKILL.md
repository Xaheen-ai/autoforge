---
name: gsd-to-xaheen-spec
description: |
  Convert GSD codebase mapping to Xaheen app_spec.txt. This skill should be used when
  the user has run /gsd:map-codebase and wants to use Xaheen on an existing project.
  Triggers: "convert to xaheen", "gsd to spec", "create app_spec from codebase",
  "use xaheen on existing project", after /gsd:map-codebase completion.
---

# GSD to Xaheen Spec Converter

Converts `.planning/codebase/*.md` (GSD mapping output) to `.xaheen/prompts/app_spec.txt` (Xaheen format).

## When to Use

- After running `/gsd:map-codebase` on an existing project
- When onboarding an existing codebase to Xaheen
- User wants Xaheen to continue development on existing code
...
```bash
mkdir -p .xaheen/prompts
```

**Mapping GSD Documents to Xaheen Spec:**

| GSD Source | Xaheen Target |
|------------|------------------|
...
```bash
cat > .xaheen/prompts/app_spec.txt << 'EOF'
...
```bash
head -100 .xaheen/prompts/app_spec.txt
echo "---"
grep -c "User can\|System\|API\|Feature" .xaheen/prompts/app_spec.txt || echo "0"
```
...
Output: .xaheen/prompts/app_spec.txt

Next: Start Xaheen

  cd {project_dir}
  python ~/projects/xaheen/start.py

Or via UI:
  ~/projects/xaheen/start_ui.sh
